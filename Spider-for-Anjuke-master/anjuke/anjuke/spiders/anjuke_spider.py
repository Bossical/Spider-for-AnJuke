...
class AnjukeSpider(Spider):
    ...
    def start_requests(self):  # 初始准备
        ...
        for index in indexlist:  # 遍历大区域
            ...
            if soup.find('div', class_='sub-items') is None:  # 判断是否存在小区域
    ...
    def parse(self, response):  # 解析页面
        ...
        for eachHouse in Houses:
            ...
            address = eachHouse.css("div.zu-info > address > a::text").extract() + eachHouse.css(
                "div.zu-info > address::text").extract()  # 拼接地址
